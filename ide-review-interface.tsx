    ...
    comments: [
      {
        ...
        message: "Consider migrating to the new makeRequest() helper for consistency with updated API patterns",
        reviewed: false,
        accepted: false,
        edited: false,
      ...
      },
      {
        ...
        severity: "❗",
        code: ["    const response = await fetch(url, {", "      method: 'POST',", "      body: JSON.stringify(data)"],
        message: "🤖 API ghosted you — try setting Content-Type!",
        reviewed: false,
        accepted: false,
        edited: false,
        manuallyResolved: false,
    ...
    comments: [
      {
        id: "3",
        severity: "⚠️",
        code: ["export async function loginUser(email: string, password: string) {", "  const response = await apiClient.post('/auth/login', {", "    email,", "    password", "  });"],
        message: "Login function should include proper error handling to catch network failures and API errors",
        reviewed: false,
        accepted: false,
        edited: false,
        manuallyResolved: false,
      ...
      },
      {
        ...
        message: "😅 Missing semicolon — DOH!",
        reviewed: false,
        accepted: false,
        edited: false,
        manuallyResolved: false,
        lineNumber: 11,
        suggestedFix: ["  localStorage.setItem('authToken', response.token);"],
      },
    ],
  },
]
